...
description: >
  ...
  /prd, /trd, /ux, /appflow, /schema, /impl, /all-docs, or asks to "create project docs",
  "write a PRD", "generate technical spec", "document my project", "write a product
  requirements document", "design the backend schema", "create an implementation plan",
  or any variation of professional project documentation. Also triggers when user says
  "I'm building X, help me document it" or "we need docs for our project". Works across
  Claude Desktop, Claude Code, Gemini CLI, OpenCode, and any agent with file access.
version: 1.0.0
author: community
license: MIT
---
...
| `/trd` | `/technical-requirements-document` | Technical Requirements Document |
| `/ux` | `/ui-ux-design` | UI/UX Design Document |
| `/appflow` | `/app-flow` | Application Flow Document |
| `/schema` | `/backend-schema` | Backend Schema Document |
| `/impl` | `/implementation-plan` | Implementation Plan |
| `/all-docs` | `/generate-all` | All 6 documents |
| `/docs-help` | `/commands` | Show this command list |
...
```
/prd --format docx
/schema --output db-design.md
/all-docs --format html
/trd MyApp is a SaaS invoicing tool for freelancers using Next.js + Supabase
```
...
2. Check if the user already provided context inline (e.g., `/prd MyApp is a...`).
...
For `/trd`, `/schema`, `/appflow` also ask:
...
| `/trd` | `references/trd.md` |
| `/ux` | `references/ux-design.md` |
| `/appflow` | `references/appflow.md` |
| `/schema` | `references/backend-schema.md` |
| `/impl` | `references/implementation-plan.md` |

For `/all-docs`, load all reference files before generating.
...
- After output, offer: "Want me to generate any of the other 5 documents next?"
...
## Multi-Document Generation (`/all-docs`)
...
4. Output each as a separate file. Also offer a combined `project-docs-bundle.md`.
